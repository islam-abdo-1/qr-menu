...
import { toImageProxyUrl, formatPrice, applyDiscount, cn, isValidPhone } from '@/lib/utils';
...
describe('formatPrice', () => {
  it('formats EGP in Arabic without decimals for whole numbers', () => {
    expect(formatPrice(100, 'EGP', 'ar')).toBe('١٠٠ ج.م');
    expect(formatPrice(0, 'EGP', 'ar')).toBe('٠ ج.م');
  });

  it('formats EGP in Arabic with decimals for fractional amounts', () => {
    expect(formatPrice(50.5, 'EGP', 'ar')).toBe('٥٠٫٥ ج.م');
  });

  it('formats EGP in English without decimals for whole numbers', () => {
    expect(formatPrice(100, 'EGP', 'en')).toBe('100 ج.م');
  });

  it('formats EGP in English with decimals for fractional amounts', () => {
